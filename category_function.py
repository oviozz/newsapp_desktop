...
class CategoryNews:
    ...
    def thread(self, list_widget, widget, gui, topic):
        news_load = Thread(target=self.add_news, args=(list_widget, widget, gui, topic))
        news_load.start()

    def add_news(self, list_widget, widget, gui, topic):

        #start = datetime.now()

        api_key = '4dbc17e007ab436fb66416009dfb59a8'
        news_call = json.loads(requests.get(f'https://newsapi.org/v2/everything?q={topic}&language=en&apiKey={api_key}').text)
        ...
        for i in range(title_count - 1):
            ...
            CategoryNews.news_link_load[title] = url

            try:
                link_requests = CategoryNews.req.get(image)
                pic = gui.QPixmap()
    ...
    def news_load_url(self, title):
        return webbrowser.open(CategoryNews.news_link_load[title])
